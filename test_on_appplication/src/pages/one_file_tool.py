...
def get_stepNo_lines(df, subfigure = False):
    ...
    if subfigure== True:
        shapes = [dict(fillcolor=color[i],
                ...
                y0=0,
                y1=0.495,
                yref="paper")  for i in range(1,len(pos))] + [dict(fillcolor=color[i],
                line={"width": 0},
                type="rect",
                x0=dff['Log Time'].iloc[i-1],
                x1=dff['Log Time'].iloc[i],
...
def make_histoboxplot(df, cols):
    ...
    for col in cols:
        ...
        fig.update_layout(dict(title=f'{col}, std:{df[col].std():.2E}\n'),showlegend=False)
...
def figs_from_store(fig1cols, fig2cols, show_trend, json_dict):
    ...
    figshapes = get_stepNo_lines(dff, subfigure=True) 
